...
**Adding MCP tools**: When adding a new tool to `coordinator/server.py`, also update `.claude-plugin/tools.json` (add entry with `needs_agent_id` flag). The sync test (`coordinator/tests/test_tools_sync.py`) catches drift between `tools.json` and `server.py`. No longer need to update `hooks/hooks.json` matcher or `hooks/ensure_agent_id.py` — they derive from `tools.json` automatically. New modules also need to be added to the `Dockerfile` COPY commands.
